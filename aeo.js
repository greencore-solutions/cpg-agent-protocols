// aeo.js — GSC AEO / discovery layer for Express MCP surfaces.
// v2 (2026-08-24, NG-7 FIX Track 1): path-specific PRM (RFC 9728), resource scopes
// replace human-login scopes, PKCE S256 + RFC 8707 resource-indicator advertisement,
// A2A 1.0 agent card (securitySchemes + scope-of-authority extension, RFC 8785
// canonical-clean), /.well-known/mcp.json, monthly transaction number cited to
// gsc-radar.ai (source of record) instead of embedded.
// v1 was 2026-08-15. ADDITIVE ONLY: registers GET routes; never touches /mcp or the transport.
...
//                   registryName, scopes, version });
//   ... app.post("/mcp", ...) ...
//   mountMcpMethodGuard(app, ghostEighteen);   // AFTER the POST route: uniform 405 JSON on every other method
import crypto from "node:crypto";
import fs from "node:fs";

// Build-time signatures (NG-7 Section C, 2026-08-24): per-host JWS over the RFC 8785
// canonical card (sans signatures), ES256 kid gsc-cards-2026-08, signed via Azure Key
// Vault API in the pipeline. Absent file = cards serve unsigned (still valid A2A 1.0).
let SIGNED_CARDS = {};
try { SIGNED_CARDS = JSON.parse(fs.readFileSync(new URL("./signed-cards.json", import.meta.url), "utf8")); } catch {}
...
const RADAR = "https://gsc-radar.ai";
const CANON = `38,350 brands / 15,688 retail banners / 3.29M points of sale / 50 markets — monthly inbound agent transaction number published at ${RADAR} (source of record)`;
...
// Live Entra mirror (verbatim keys, capability advertisement augmented like grant_types):
// PKCE S256 is native Entra v2 behavior; RFC 8707 resource indication is honored via the
// Entra v2 mapping — the resource URI travels as the scope prefix (scope=<resource>/.default).
const ENTRA = {
...
export function mountAeo(app, cfg) {
  ...
  // Resource scopes (NG-7 FIX item 3): what a token presented at THIS resource carries.
  // Human-login scopes (openid/profile/email/offline_access) are AS-level, not resource scopes — out.
  const SCOPES = cfg.scopes || { "mcp:read": `Read/resolve/discovery calls on ${title} over MCP.` };
  const REGISTRY = cfg.registryName || "io.github.greencore-solutions/cpg-knowledge-graph";
  ...
  app.get("/sitemap.xml", (req, res) => send(res, req, "application/xml",
    `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n  <url><loc>https://${H(req)}/</loc><lastmod>2026-08-24</lastmod></url>\n</urlset>\n`));
...
`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>${title} — ${h}</title><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="canonical" href="https://${h}/"><style>body{font-family:Inter,system-ui,sans-serif;color:#141414;max-width:760px;margin:60px auto;padding:0 24px;line-height:1.6}h1{font-family:Archivo,sans-serif}code{background:#F7F6F3;padding:2px 6px;border-radius:4px}a{color:#F38020}</style></head><body><h1>${title}</h1><p>${desc}</p><p>MCP endpoint (streamable-http): <code>${mcpUrl(h)}</code></p><p>Machine files: <a href="/llms.txt">llms.txt</a> · <a href="/index.md">markdown</a> · <a href="/.well-known/mcp.json">mcp.json</a> · <a href="/.well-known/mcp/server-card.json">MCP server card</a> · <a href="/.well-known/agent-card.json">A2A agent card</a> · <a href="/.well-known/agent-skills/index.json">agent skills</a> · <a href="/health.json">health</a></p><p>Operator: ${OP} · D-U-N-S 24-336-6774 · <a href="https://gsc-navigator.ai/">Human-in-the-Loop</a></p><script src="/webmcp.js" defer></script></body></html>`);
  ...
  app.get("/.well-known/mcp/server-card.json", (req, res) => json(res, req, {
    ...
    capabilities: { tools: {} }, tools, operator: OP, registry: REGISTRY }));
  // /.well-known/mcp.json (NG-7 FIX item 2, receipt evenness): same shape as the KG's.
  app.get("/.well-known/mcp.json", (req, res) => json(res, req, {
    name: REGISTRY, title, description: desc, version: cfg.version || "1.0.0",
    operator: OP, remotes: [{ type: "streamable-http", url: mcpUrl(H(req)) }],
    tools, protocol, sovereign_manifest: "SM-ECO-10060" }));
  ...
  // RFC 9728 Protected Resource Metadata — root form (resource = the origin) and
  // path-inserted form for /mcp (what a strict 2026 MCP client derives from the endpoint URL).
  const prm = (h, path = "") => ({ resource: `https://${h}${path}`, resource_name: `${title}${path ? " (MCP endpoint)" : ""}`,
    authorization_servers: [ISSUER], scopes_supported: Object.keys(SCOPES),
    bearer_methods_supported: ["header"], resource_documentation: `https://${h}/auth.md`,
    enforcement_posture: SIGN_LINE });
  app.get("/.well-known/oauth-protected-resource", (req, res) => json(res, req, prm(H(req))));
  app.get("/.well-known/oauth-protected-resource/mcp", (req, res) => json(res, req, prm(H(req), "/mcp")));
...
- Resource: https://${h}
- MCP resource: ${mcpUrl(h)}
- Protected Resource Metadata (RFC 9728): [/.well-known/oauth-protected-resource](/.well-known/oauth-protected-resource) · path-specific: [/.well-known/oauth-protected-resource/mcp](/.well-known/oauth-protected-resource/mcp)
...
Human-login scopes (openid, profile, email, offline_access) are authorization-server scopes and are not resource scopes on this surface.

## PKCE and resource indication

- Authorization-code requests use **PKCE** (\`code_challenge_methods_supported\`: S256).
- **RFC 8707 resource indication**: the resource URI for this surface's MCP endpoint is ${mcpUrl(h)}. At the Microsoft Entra v2 endpoint the resource is carried as the scope prefix (\`scope=<resource>/.default\`); clients that send the \`resource\` parameter directly are interoperable via that mapping.
...
{name:"gsc_stats",description:"Canonical GSC network statband (public set). Monthly transaction number is published at gsc-radar.ai — the source of record.",inputSchema:{type:"object",properties:{}},execute:async function(){return {brands:38350,retail_banners:15688,points_of_sale:3290000,markets:50,monthly_transactions_source_of_record:"${RADAR}",operator:"${OP}",mcp:"${MCP_KG}"};}}
...
`); });
  // A2A agent card — current released shape verified on the wire 2026-08-24: A2A 1.0
  // (a2aproject/A2A v1.0.1; specification/a2a.proto is normative). securitySchemes declared,
  // scope-of-authority declared machine-readably as a capability extension, body is static
  // per-host (no timestamps/nonces) so it is RFC 8785 canonical-clean and signature-ready
  // (signatures[] lands when key custody is ruled — Track 2).
  // REPLACES the legacy card route: register FIRST so it wins.
  app.get("/.well-known/agent-card.json", (req, res) => { const h = H(req); json(res, req, {
    name: title,
    description: desc,
    supportedInterfaces: [
      { url: mcpUrl(h), protocolBinding: "HTTP+JSON", protocolVersion: "1.0" },
    ],
    provider: { url: "https://gsc-em.com", organization: OP },
    version: cfg.version || "1.0.0",
    documentationUrl: `https://${h}/llms.txt`,
    capabilities: {
      streaming: true,
      ...
      extensions: [{
        ...
        required: false,
        params: {
          autonomous: ["read", "resolve", "discover"],
          humanSigned: ["transactional-commit", "order", "rfq-acceptance", "terms"],
    ...
    securitySchemes: {
      entraOAuth2: { oauth2SecurityScheme: {
        ...
        flows: { clientCredentials: { tokenUrl: TOKEN_URL, scopes: SCOPES } },
        oauth2MetadataUrl: `https://${h}/.well-known/oauth-authorization-server`,
      } },
      bearerJwt: { httpAuthSecurityScheme: { scheme: "Bearer", bearerFormat: "JWT",
    ...
    ...(SIGNED_CARDS[h] ? { signatures: SIGNED_CARDS[h] } : {}),
  }); });
}

// ---- THE SIGN (NG-7 amendment A1, CEO ruling 2026-08-24) ----------------------
// All methods stay OPEN. No challenges deploy now. This dated, machine-readable
// enforcement declaration publishes instead — declaration only, no behavior change.
export const SIGN = {
...
export const SIGN_HUMAN = "These endpoints are open by design. When the buy side's credential standard reaches GA — expected Q4 2026-Q1 2027 — transactional methods migrate to OAuth 2.1 challenge per the MCP 2025-11-25 specification. Read access stays open. That is not pending work; that is the published plan.";
export const SIGN_LINE = "open-by-declaration · migration: OAuth 2.1 + PKCE challenge (401 + WWW-Authenticate) on mcp:transact methods per MCP 2025-11-25 · trigger: ecosystem GA of credentialed counterparties · window: Q4 2026 - Q1 2027 · declared 2026-08-24";
export const SIGN_EXT = {
  uri: "https://gsc-em.com/a2a/ext/enforcement-posture/v1",
  description: SIGN_HUMAN,
  required: false,
  params: SIGN,
};

// One /mcp method convention across the Express MCP servers (NG-7 FIX item 2):
// POST is JSON-RPC 2.0 (MCP streamable-http); every other method answers a deliberate
// JSON 405 with an Allow header — never a default HTML 404. Mount AFTER app.post("/mcp").
// (The stateful KG follows streamable-http spec semantics: GET is the SSE channel with
// the proper Accept header, DELETE terminates sessions — deliberate JSON on every method.)
export function mountMcpMethodGuard(app, ghost = () => {}) {
  app.all("/mcp", (req, res) => {
    ghost(res);
    res.set("Allow", "POST");
    res.status(405).json({ error: "method_not_allowed", allow: ["POST"],
      meaning: "POST JSON-RPC 2.0 (MCP streamable-http) to /mcp." });
  });
}
